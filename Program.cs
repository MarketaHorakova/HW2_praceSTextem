...
it was the grayish white of old porridge.";
        ...
        Console.WriteLine(delkaTextu); //1024
        ...
        // Zjisti, jestli je v textu zminka o obtloustle "tete" Harryho. Jmenuje se Marge.
        // Vysledek uloz do promenne 'piseSeOMarge';
        ...
        // Pomoci abecedniho porovnavani zjisti, ktery z nasledujicich textu je podle abecedy prvni
        // a jeho hodnotu prirad do promenne 'prvni'.

        string blabol1 = "abbc";
        string blabol2 = "acbc";
        string blabol3 = "abbb";
      
        string[] poleStringu = new string[] {blabol1,blabol2,blabol3};
        int j = poleStringu.Length-1;

        for (int k = 0; k < 2; k++)
        {
            for (int i = 0; i < poleStringu.Length - 1; i++)
            {

                if (string.Compare(poleStringu[i], poleStringu[i + 1]) == 1)
                {
                    var variable = poleStringu[i];
                    poleStringu[i] = poleStringu[i + 1];
                    poleStringu[i + 1] = variable;
                }

            }
        }

        string prvni = poleStringu[0];
        ...
        bool vyskytujeSeAnd = text.Contains(" and ",StringComparison.InvariantCultureIgnoreCase);

        string[] pocetTextuMeziAnd = text.Split(" and ");
        int pocetAnd = pocetTextuMeziAnd.Length;
